...
void make_chunk(uint32_t fp, file_layout **pFileLayout){
    ...
    (**pFileLayout).pType_ids = (uint32_t *)mmap(0, sizeof(type_id_item) * pHeader->type_ids_size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
	...
	read_chunk(fp, (**pFileLayout).pType_ids, pHeader->type_ids_off, sizeof(type_id_item) * pHeader->type_ids_size);
...
void delete_chunk(file_layout **pFileLayout){
	...
	munmap((**pFileLayout).pType_ids, sizeof(type_id_item) * pHeader->type_ids_size);
